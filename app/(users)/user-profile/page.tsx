...
import { MapPin, Briefcase, Heart, Cake, Mail, Phone, Globe, Twitter, Linkedin, Github, Users, Coffee, GraduationCap } from 'lucide-react'
...
export default function UserProfile() {
  const user = {
    ...
    favoritePlaces: ["Marine Drive", "Elephanta Caves", "Sanjay Gandhi National Park"],
      ...
      <Card className="w-full max-w-4xl mx-auto shadow-2xl">
        <CardHeader className="pb-4">
          <div className="flex flex-col md:flex-row items-center md:items-start space-y-4 md:space-y-0 md:space-x-6">
            ...
            <div className="text-center md:text-left">
              <CardTitle className="text-3xl md:text-4xl font-bold text-purple-800">{user.name}</CardTitle>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <h3 className="text-xl font-semibold mb-3 text-purple-800">Personal Information</h3>
              <div className="space-y-2">
                ...
                <div className="flex items-center">
                  ...
                  <a href={`https://${user.website}`} className="text-blue-600 hover:underline">{user.website}</a>
                </div>
              </div>
            </div>
            <div>
              <h3 className="text-xl font-semibold mb-3 text-purple-800">Social Media</h3>
              <div className="space-y-2">
                <div className="flex items-center">
                  <Twitter className="w-5 h-5 mr-2 text-blue-400" />
                  <a href={`https://twitter.com/${user.twitter.slice(1)}`} className="text-blue-600 hover:underline">{user.twitter}</a>
                </div>
                <div className="flex items-center">
                  <Linkedin className="w-5 h-5 mr-2 text-blue-700" />
                  <a href={`https://www.linkedin.com/in/${user.linkedin}`} className="text-blue-600 hover:underline">{user.linkedin}</a>
                </div>
                <div className="flex items-center">
                  <Github className="w-5 h-5 mr-2 text-gray-700" />
                  <a href={`https://github.com/${user.github}`} className="text-blue-600 hover:underline">{user.github}</a>
                </div>
              </div>
            </div>
          </div>
          ...
          <div>
            <h3 className="text-xl font-semibold mb-3 text-purple-800">Languages</h3>
            <div className="flex flex-wrap gap-2">
              {user.languages.map((language, index) => (
                <Badge key={index} variant="secondary" className="bg-green-100 text-green-800">{language}</Badge>
              ))}
            </div>
          </div>
          <div className="mt-6">
            <h3 className="text-xl font-semibold mb-3 text-purple-800">Hobbies</h3>
            <div className="flex flex-wrap gap-2">
              {user.hobbies.map((hobby, index) => (
                <Badge key={index} variant="secondary" className="bg-orange-100 text-orange-800">{hobby}</Badge>
              ))}
            </div>
          </div>
          <div className="mt-6">
            <h3 className="text-xl font-semibold mb-3 text-purple-800">Favorite Mumbai Experiences</h3>
          ...
          <div className="mt-6">
            <h3 className="text-xl font-semibold mb-3 text-purple-800">Full Description</h3>
            <p className="text-gray-700 whitespace-pre-line">{user.fullDescription}</p>
          </div>
        </CardContent>
      </Card>
    </div>
