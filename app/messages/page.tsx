...
export default function UserMessages() {
  ...
  const chats = [
    ...
    { id: 2, name: 'Jane Smith', lastMessage: 'Can we meet tomorrow?', timestamp: 'Yesterday' },
    { id: 3, name: 'Admin Support', lastMessage: 'Your request has been processed.', timestamp: '2 days ago' },
  ]
  ...
  const messages = [
    ...
    { id: 3, sender: 'John Doe', content: 'Doing well! Just wanted to check in.', timestamp: '10:32 AM' },
  ]
            ...
            {chats.map((chat) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <Avatar>
                    <AvatarImage src={`/placeholder.svg?text=${chat.name[0]}`} />
          ...
          {selectedChat ? (
            <>
              <div className="p-4 border-b flex justify-between items-center">
                <div className="flex items-center gap-3">
                  <Avatar>
                    <AvatarImage src={`/placeholder.svg?text=${chats.find(c => c.id === selectedChat)?.name[0]}`} />
              ...
              <ScrollArea className="flex-1 p-4">
                {messages.map((msg) => (
                  <div key={msg.id} className={`mb-4 ${msg.sender === 'You' ? 'text-right' : ''}`}>
                    <div className={`inline-block p-2 rounded-lg ${msg.sender === 'You' ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}>
                      <p>{msg.content}</p>
